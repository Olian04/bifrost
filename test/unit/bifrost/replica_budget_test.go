...
func TestPlanReplicaCounts_explicitOnly(t *testing.T) {
	t.Parallel()
	snap := bifrost.SystemSnapshot{GOMAXPROCS: 8, AvailableBytes: 16 << 30}
	cfg := []int{3, 7}
	part := []int{0, 0}
	got, note, err := bifrost.PlanReplicaCounts(cfg, part, snap)
	if err != nil {
		t.Fatal(err)
	}
	if got[0] != 3 || got[1] != 7 {
...
func TestPlanReplicaCounts_autoFitsUnderGlobalCap(t *testing.T) {
	t.Parallel()
	snap := bifrost.SystemSnapshot{GOMAXPROCS: 8, AvailableBytes: 64 << 30}
	cfg := []int{0, 0}
	// Per-bridge cap at GOMAXPROCS=8 is min(4096, max(4, 64)) = 64
	part := []int{30, 40}
	got, _, err := bifrost.PlanReplicaCounts(cfg, part, snap)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPlanReplicaCounts_autoFairShareWhenOverCap(t *testing.T) {
	t.Parallel()
	snap := bifrost.SystemSnapshot{GOMAXPROCS: 4, AvailableBytes: 128 << 30}
	n := 20
	cfg := make([]int, n)
	part := make([]int, n)
	for i := range cfg {
...
func TestComputeGlobalRelayCap_atLeastBridgeCount(t *testing.T) {
	t.Parallel()
	if g := bifrost.ComputeGlobalRelayCap(5, bifrost.SystemSnapshot{GOMAXPROCS: 1, AvailableBytes: 0}); g < 5 {
		t.Fatalf("got %d", g)
	}
}

...
func TestBridgeRunOptions_UsesConfiguredRetryDurations(t *testing.T) {
	...
	opts, err := bifrost.BridgeRunOptions(15*time.Second, bridgeCfg, fromCluster, toCluster)
	if err != nil {
...
func TestBridgeRunOptions_UsesRetryDefaultsWhenOmitted(t *testing.T) {
	opts, err := bifrost.BridgeRunOptions(0, config.Bridge{}, config.Cluster{}, config.Cluster{})
	if err != nil {
		t.Fatalf("BridgeRunOptions: %v", err)
	}

...
func TestValidatePreservedPartitionCounts_WithOverridePartition(t *testing.T) {
	...
	if err := bifrost.ValidatePreservedPartitionCounts(bridgeCfg, 10, 3); err != nil {
		t.Fatalf("expected success, got %v", err)
	}
	if err := bifrost.ValidatePreservedPartitionCounts(bridgeCfg, 10, 2); err == nil {
...
func TestValidatePreservedPartitionCounts_PreserveMode(t *testing.T) {
	...
	if err := bifrost.ValidatePreservedPartitionCounts(bridgeCfg, 6, 6); err != nil {
		t.Fatalf("expected equal partitions to pass, got %v", err)
	}
	err := bifrost.ValidatePreservedPartitionCounts(bridgeCfg, 6, 5)
	if err == nil {
